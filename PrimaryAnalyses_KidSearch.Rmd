...
```{r}
numSubs = length(unique(exp.tidy$sub))
halfSubs = round(numSubs/2)

dz_splitHalf = array()
condEffect_splitHalf = array()
for (iter in 1:100){
  theseSubs = sample(unique(exp.tidy$sub),halfSubs)
...
if (expName=="Edibility"){
  levels(plotSummary$category) = c("Edible Targets","Inedible Targets")
  levels(plotSummary$conditionName) = c("Uniform Edibility","Mixed Edibility")
  relevel(plotSummary$conditionName,ref="Uniform Edibility")}
